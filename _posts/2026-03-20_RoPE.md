...
Firstly, consider $q, k \in \mathbb{C}$ or $\mathbb{R}^2$ distinguished only by the context. Obviously, $\langle q, k \rangle = \operatorname{Re}(qk^*).$

So the former equation becomes:

$$g(q, k, m - n) = \operatorname{Re}(f(q, m) f^*(k, n)).$$
...
$$f(q, m) = \|q\|\, e^{\hat{i}(\Theta(q) + m\theta)} = q\, e^{\hat{i} m\theta} \rightarrow \begin{pmatrix} \cos(m\theta) & -\sin(m\theta)\\ \sin(m\theta) & \cos(m\theta) \end{pmatrix} \begin{pmatrix} \operatorname{Re}(q)\\ \operatorname{Im}(q) \end{pmatrix},$$

Tricky thing is, $\theta = \Theta_g(q, k, 1) + \Theta(k) - \Theta(q),$ but $g$ is still an abstract-defined function. So $\theta$ is chosen from another way: $\theta_i = \dfrac{1}{10000^{-\frac{2(i-1)}{d}}},\ i \in \left[\dfrac{d}{2}\right]$, $\operatorname{token} \in \mathbb{R}^d$.
