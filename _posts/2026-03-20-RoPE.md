...
$$\left\{ \begin{array}{l} \tilde{q}_m = f(q, m),\\ \tilde{k}_n = f(k, n). \end{array} \right.$$
...
$$\begin{cases} f(\ast, n) = R_f(\ast, n), e^{\hat{i} \Theta_f(\ast, m)},\ g(q, k, m - n) = R_g(q, k, m-n), e^{\hat{i} \Theta_g(q, k, m-n)}. \end{cases}$$
...
$$R_f(q, m)\spaceR_f(k, m) = R_g(q, k, 0) = R_f(q, 0)\space R_f(k, 0) = |q|\space |k|.$$
...
$$f(q, m) = |q|\space e^{\hat{i}(\Theta(q) + m\theta)} = q\space e^{\hat{i} m\theta}.$$
